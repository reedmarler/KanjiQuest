...
function reviewedContentRecord(word: CategoryWordRecord, tags: string[], group: TagGroupName, database: ContentRecord[], preferredTranslation=word.preferredTranslation): ContentRecord {
  ...
  return {
    ...
    preferredTranslation:preferredTranslation.trim() || existing?.preferredTranslation || inferPreferredTranslation(word.japanese,word.english),
...
function generateTestSentence(levels: JlptLevel[], turn: number) {
  ...
  const requestedFrame=level === 'N5' ? undefined : `${level.toLowerCase()}-${String((levelTurn % 10)+1).padStart(2,'0')}`
...
export function ContentStudio({ onBack }: { onBack: () => void }) {
  ...
  function saveVocabulary(event: React.FormEvent<HTMLFormElement>) {
    ...
    const preferredTranslation = String(form.get('preferredTranslation') ?? '').trim() || inferPreferredTranslation(japanese,english)
