...
export function KanaChartPanel({ script, onOpenQuiz, onSwitchScript, onSelectCharacter }: KanaChartPanelProps) {
  ...
  const headingTitle = script === 'hiragana' ? deck.title : `${deck.title} Chart`
  ...
  return (
    <>
      ...
      <section className={`hiragana-chart-board${showRomaji ? '' : ' is-romaji-hidden'}`} aria-label={`${deck.title} rows`}>
