/*
 * A copy of GrammarPractice, to be modernised here before anything
 * changes in the tool people actually use.
 *
 * It is a copy on purpose. It shares no code with the original, so a rewrite
 * in here cannot reach the live screen — and when a change is worth keeping,
 * the diff between this file and its origin is the change to make there.
 *
 * Its markup and class names still match the original, so it opens looking
 * exactly like the real thing; see `.tool-lab` in App.css for how to restyle
 * it without touching the tool it was copied from.
 */
...
import { ChoiceDrillLab, loadLevelPreference } from './ChoiceDrillLab'
...
export function GrammarPracticeLab({ onBack, onDashboard, isFavorite, onToggleFavorite, questId, onQuestComplete, furiganaDefault }: GrammarPracticeLabProps) {
